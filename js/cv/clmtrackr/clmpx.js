...
var clmpm = {
	tracker : function(params) {
		...
		*/
		...
		var minFaceSize = 0.2; // minimum face size wrt canvas size
		...
		// track every frame in a video
		this.trackFrame = function(element, box) {
			// check if model is initalized, else return false
			if (typeof(model) === "undefined") {
				console.log("tracker needs to be initalized before starting to track.");
				return false;
		...
		/*
		 *	draw model on given canvas: flip the results
		 */
		this.flipdraw = function(canvas, w, h, pv, path) {
			...
			for (var i = 0;i < paths.length;i++) {
				if (typeof(paths[i]) == 'number') {
					...
					drawFlipedPoint(cc, paths[i], params, w, h);
				} else {
					if (numig > 0){
						numig--;
					}else{
						// drawPath(cc, paths[i], params);
						drawFlipedPath(cc, paths[i], params, w, h);
		...
		var detectPosition = function(el) {
			...
			// **PM**
			// ignore small face boxes
			
			if (candidate.width < minFaceSize*el.width){
				return false;
			}
		...
		// draw a fliped parametrized line on a canvas
		var drawFlipedPath = function(canvasContext, path, dp, w, h) {
			canvasContext.beginPath();
			var i, x, y, a, b;
			for (var p = 0;p < path.length;p++) {
				...
				if (i == 0) {
					canvasContext.moveTo(w-x,y);
				} else {
					canvasContext.lineTo(w-x,y);
				}
			}
		...
		// draw a fliped point on a canvas
		function drawFlipedPoint(canvasContext, point, dp, w, h) {
			var i, x, y, a, b;
			i = point*2;
			x = meanShape[i/2][0];
			y = meanShape[i/2][1];
			...
			y += b;
			canvasContext.beginPath();
			canvasContext.arc(w-x, y, 1, 0, Math.PI*2, true);
			canvasContext.closePath();
			canvasContext.fill();
		}
